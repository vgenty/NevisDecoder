...
#include "algo_fem_decoder_base.h"
...
namespace larlite {
  ...
  //#################################################
  {
    ...
    _algo_fem_last_word = fem::kFEM_LAST_WORD;
    reset();
  }
  ...
  //#################################################
  {
    ...
    case fem::kFEM_HEADER:
      ...
      // (1) Check if the last word was kEVENT_HEADER, kFEM_HEADER, or kEVENT_LAST_WORD
      //     - If kEVENT_HEADER, continue to (2)
      //     - If kFEM_HEADER,   continue to (2)
      //     - If kEVENT_LAST_WORD, attempt to store & continue to (2)
    ...
    case fem::kUNDEFINED_WORD: 
      ...
      if(word != 0x0 || (last_word_class != fem::kEVENT_LAST_WORD && last_word_class != fem::kFEM_LAST_WORD) ) {
	...
	if(_header_info.event_number>0)
	  ...
	  Message::send(msg::kWARNING,__FUNCTION__,Form("Failed decoding event %d ...",_header_info.event_number));

	Message::send(msg::kWARNING,__FUNCTION__,"kDEBUG MODE => Continue to the next event...\n");
  ...
  //##################################################################################
  {
	...
	store_event();
      ...
      else if( last_word_class != fem::kFEM_HEADER && last_word_class != fem::kEVENT_HEADER ) {
		
	// kERROR
	Message::send(msg::kERROR,__FUNCTION__,
		      Form("Unexpected word while kFEM_HEADER processing: %x (previous word=%x)",word,last_word));
	status = false;
      }
	...
	if (_event_header_count<kFEM_HEADER_COUNT) {
	  ...
	  Message::send(msg::kERROR,__FUNCTION__,
			Form("Logic error: event header word counter not working! (%zu/%zu)",
			     _event_header_count,kFEM_HEADER_COUNT));
	  status=false;
  ...
  //#################################################
	...
	Message::send(msg::kINFO, __FUNCTION__, Form("Trigger Frame %d",_header_info.fem_trig_frame_number));
	Message::send(msg::kINFO, __FUNCTION__, Form("Trigger Sample %d",_header_info.fem_trig_sample_number));
      }
  ...
  //#################################################
  {
    ...
    //    else if (nbits==4) {nbits=3; diff = 0x7; mask = 0x7;}
      ...
      print(msg::kERROR,__FUNCTION__,"Only supported for nbits = [3,4]!");
	...
	print(msg::kERROR,__FUNCTION__,Form("Unexpected diff: ref=%d, subject=%d",ref,subject));
	throw decode_algo_exception();
      }
