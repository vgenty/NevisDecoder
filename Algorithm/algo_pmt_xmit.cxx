...
namespace larlite {
  ...
  //#########################################
    ...
    if(status){
      _nwords++;
      //if(get_word_class(word) == fem::kCHANNEL_WORD) 
  ...
  //#################################################
    ...
    if(!_event_data)
      ...
      _event_data = (event_pmtfifo*)(_storage->get_data<event_pmtfifo>("pmt_fifo"));
  ...
  //#########################################################
    ...
    if(_verbosity[msg::kINFO])
      ...
      Message::send(msg::kINFO,__FUNCTION__,Form("End of FEM word: %x...",word));

    UInt_t last_word_class = get_word_class(last_word);
    if(last_word_class != fem::kCHANNEL_LAST_WORD &&
       last_word_class != fem::kFEM_FIRST_WORD &&
  ...
  //#########################################################
    ...
    if(_verbosity[msg::kINFO])
      ...
      Message::send(msg::kINFO,__FUNCTION__,Form("End of event word: %x...",word));

    if(get_word_class(last_word)!=fem::kFEM_LAST_WORD){
  ...
  //#########################################################
    ...
    case fem::kCHANNEL_HEADER:
      ...
      if(last_word_class!=fem::kFEM_FIRST_WORD && 
	...
	// Discriminator ID
	_ch_data.set_disc_id((fem::Discriminator_t)((word & 0xfff)>>9));  
      }
      break;
	...
	//
	if(_channel_header_count==kCHANNEL_HEADER_COUNT ) {
	  ...
	  else if(word_class!=fem::kCHANNEL_LAST_WORD)
	  ...
	  if(_header_info.event_frame_number == fem::kINVALID_WORD) {
	    print(msg::kERROR,__FUNCTION__,Form("Invalid event frame number: %d",fem::kINVALID_WORD));
	    return false;
	  }
	  ...
	  if(diff < 0 || diff > 3) {
	    print(msg::kWARNING,__FUNCTION__,Form("Found event frame %d and discriminator frame %d (difference too big!)",
	  ...
	  if(_verbosity[msg::kINFO]){
	    ...
	    Message::send(msg::kINFO,_buf);
	  }
	}

	// If this is channel's last word, store & clear channel info
	if(word_class==fem::kCHANNEL_LAST_WORD){
	  if(_verbosity[msg::kINFO]){
	    Message::send(msg::kINFO,Form("Encountered the last word (%x) for channel %d",word,_ch_data.channel_number()));
	    Message::send(msg::kINFO,Form("Event frame  : %d",_header_info.event_frame_number));
	    Message::send(msg::kINFO,Form("PMT frame    : %d",_ch_data.readout_frame_number()));
	    Message::send(msg::kINFO,Form("Disc. ID     : %d",_ch_data.disc_id()));
	    Message::send(msg::kINFO,Form("Start Time   : %d",_ch_data.readout_sample_number_64MHz()));
	    Message::send(msg::kINFO,Form("# ADC sample : %zd",_ch_data.size()));
	  }
	  store_ch_data();
	}
	// else check fifo overflow
	  ...
	  Message::send(msg::kERROR,"Bit marker 0x8xxx not found at expected position (possible FIFO overflow)!");
	  return false;
	}
      }
      break;
