#ifndef ALGO_SLOW_READOUT_DECODER_CXX
#define ALGO_SLOW_READOUT_DECODER_CXX
...
namespace larlite {
  ...
  void algo_slow_readout_decoder::init_event_info(){
    ...
    _last_channel_number=fem::kINVALID_CH;
    _last_disc_id=fem::kDISC_MAX;
    _beam_event=false;
    _beam_ref_sample=fem::kINVALID_WORD;
    _beam_ref_nwords=fem::kINVALID_WORD;
    _beam_ref_frame=fem::kINVALID_WORD;
    _process=READ_HEADER;
  ...
  void algo_slow_readout_decoder::print_adc_values()
  {
	...
	ctr++;
	if(ctr%8==0) {
	  Message::send(msg::kINFO,line.c_str());
	  line="";
	}
      }
  }
  ...
  bool algo_slow_readout_decoder::process_word(UInt_t word) {
    ...
    if(!_event_data) {
      ...
      if(!_event_data) {
	Message::send(msg::kERROR,__FUNCTION__,"Could not retrieve pmt_wf_collection poitner!");
	return false;
      }
    }
    ...
    case READ_CHANNEL:
      ...
      if(word_class==fem::kFEM_HEADER || 
	 word_class==fem::kEVENT_HEADER) {
	...
	if(_debug_mode) {
	  ...
	  if(!status){
	    sprintf(_buf,"kDEBUG mode-> continue ... but Skip saving event: %d ",_event_data->event_number());
	    Message::send(msg::kWARNING,__FUNCTION__,_buf);
	  }else{
	    sprintf(_buf,"kDEBUG mode-> continue ... Good checksum -> stored previous event: %d",_event_data->event_number());
	    Message::send(msg::kWARNING,__FUNCTION__,_buf);
	    _storage->next_event();
	  }
	  status=process_header(word);
  ...
  bool algo_slow_readout_decoder::process_header(const UInt_t word) {
	...
	if(!_debug_mode)
	  status=false;
      }else if(_verbosity[msg::kWARNING]){
    ...
    case fem::kFEM_HEADER:
      ...
      if(get_word_class(word>>16)!=fem::kFEM_HEADER) {
  ...
  bool algo_slow_readout_decoder::process_ch_word(const UInt_t word,
    ...
    if(word_class==fem::kUNDEFINED_WORD){
	 ...
	 last_word_class!=fem::kFEM_LAST_WORD &&
    ...
    if(!status){
      ...
      if(word_class==fem::kEVENT_LAST_WORD){
	...
	init_event_info();
      }else if(word_class!=fem::kFEM_LAST_WORD){
	_nwords-=1;
	_checksum-=word;
      }
  ...
  bool algo_slow_readout_decoder::decode_ch_word(const UInt_t word, 
    ...
    case fem::kFEM_HEADER:
      ...
      Message::send(msg::kERROR,__FUNCTION__,_buf);
      Message::send(msg::kERROR,__FUNCTION__,"Terminate channel data readout loop.");
      status=false;
      break;
    ...
    case fem::kCHANNEL_HEADER:
      ...
      if(last_word_class!=fem::kFEM_FIRST_WORD && last_word_class!=fem::kCHANNEL_LAST_WORD &&
	...
	_ch_data.set_disc_id((fem::Discriminator_t)((word & 0xfff)>>9));  // Upper-most 2 bits of 12-bit word is discriminator ID
      }
      break;
	...
	if(last_word_class==fem::kCHANNEL_LAST_WORD){
	  ...
	  if( word == 0x8000 ) {
	    ...
	    // Do an operation that is done for the case of kCHANNEL_HEADER, 
