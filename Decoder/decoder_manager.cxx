#ifndef DECODER_MANAGER_CXX
#define DECODER_MANAGER_CXX
...
namespace larlite {
      ...
      _fin(FORMAT::ASCII)
  {
    ...
    _storage->set_io_mode(storage_manager::kWRITE);

    std::cout << "storage manage: set_io_mode: " << storage_manager::kWRITE << std::endl;
  ...
  bool decoder_manager::initialize()
  {
    if(_verbosity[msg::kDEBUG])
      Message::send(msg::kDEBUG,__FUNCTION__," begins...");
    if(!_decoder){
      Message::send(msg::kERROR,__FUNCTION__,"Algorithm not attached. Aborting.");
      return false;
    }
    if(!_storage){
      Message::send(msg::kERROR,__FUNCTION__,"Storage I/O pointer is empty.");
      return false;
    }

    Message::send(msg::kINFO,__FUNCTION__,Form("Verbosit level is %d",_verbosity_level));
    ...
    if(_fin.is_open())
      Message::send(msg::kWARNING,"Reading alrady-opened file contents!");
    else if(!_fin.open()) {
      Message::send(msg::kERROR,__FUNCTION__,"Failed file I/O...");
      status=false;
    }
  ...
  bool decoder_manager::decode(UInt_t limit) {
	...
	//_decoder->backtrace();
	if(_debug_mode){
	  Message::send(msg::kERROR,__FUNCTION__,"Process status failure ... but continue since kDEBUG mode!");
	  status=true;
	}
      }
    ...
    if(!status && !_debug_mode){
      ...
      Message::send(msg::kERROR,__FUNCTION__,Form("Event loop terminated. Stored: %d events",_storage->get_entries()));

    }else if(!(_decoder->is_event_empty())) {

      Message::send(msg::kWARNING,__FUNCTION__,"Last event not stored by algorithm. Missing end-of-event word??");

      auto event_quality =_decoder->check_event_quality();
	
      Message::send(msg::kWARNING,__FUNCTION__,Form("Event quality is %d",event_quality));

      if(event_quality) {

	Message::send(msg::kWARNING,__FUNCTION__,"Last event checksum agreed. Saving on file...");

	_storage->next_event();

      } else if(_fin.eof()) {

	Message::send(msg::kWARNING,__FUNCTION__,"Skip saving the last event...");
	...
	Message::send(msg::kERROR,__FUNCTION__,"Encountered a bad event before eof! Terminating...");
	status = false;
