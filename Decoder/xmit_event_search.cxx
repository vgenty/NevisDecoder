#ifndef XMIT_EVENT_SEARCH_CXX
#define XMIT_EVENT_SEARCH_CXX

#include "xmit_event_search.h"
...
namespace larlite {
  ...
  bool xmit_event_search::run() {
    ...
    if(_target_id==fem::kINVALID_WORD){
      Message::send(msg::kERROR,__FUNCTION__,"Event ID not specified!");
      return false;
    }
    ...
    std::vector<unsigned> word_array(4,0);
    unsigned word=0x0;
    bool fire=false;

    _fin.set_verbosity(msg::kERROR);
    _fin.open();
  ...
  void xmit_event_search::print_word(std::vector<unsigned> *in_array){

    size_t ctr=0;
    std::string msg("");

    std::vector<unsigned> word_array;
    word_array.reserve(in_array->size() * 2);
    for(std::vector<unsigned>::const_iterator iter(in_array->begin());
	iter!=in_array->end();
	++iter){
    ...
    for(std::vector<unsigned>::const_iterator iter(word_array.begin());
	iter!=word_array.end();
	++iter){
