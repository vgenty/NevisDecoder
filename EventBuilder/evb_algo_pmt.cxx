#ifndef EVB_ALGO_PMT_CXX
#define EVB_ALGO_PMT_CXX
...
namespace larlite {
  ...
  {
    print(msg::kDEBUG,__FUNCTION__,Form("Building %d %d...",id,_current_event_number));
    ...
    if(out_event_pmtfifo->checksum() == fem::kINVALID_WORD) out_event_pmtfifo->set_checksum(0);
    if(out_event_pmtfifo->nwords()   == fem::kINVALID_WORD) out_event_pmtfifo->set_nwords(0);
    ...
    if(out_event_pmtfifo->event_frame_number() == fem::kINVALID_WORD)

      out_event_pmtfifo->set_event_frame_number ( in_event_pmtfifo->event_frame_number() );

    if(out_event_pmtfifo->fem_trig_frame_number() == fem::kINVALID_WORD)

      out_event_pmtfifo->set_fem_trig_frame_number ( in_event_pmtfifo->fem_trig_frame_number() );

    if(out_event_pmtfifo->fem_trig_sample_number_RAW() == fem::kINVALID_WORD)
