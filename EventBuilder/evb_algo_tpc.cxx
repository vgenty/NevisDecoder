#ifndef EVB_ALGO_TPC_CXX
#define EVB_ALGO_TPC_CXX
...
namespace larlite {
  ...
  bool evb_algo_tpc::build(storage_manager *out_storage, UInt_t id)
  {
    ...
    if(out_event_tpcfifo->checksum() == fem::kINVALID_WORD) out_event_tpcfifo->set_checksum(0);
    if(out_event_tpcfifo->nwords()   == fem::kINVALID_WORD) out_event_tpcfifo->set_nwords(0);
    ...
    if(out_event_tpcfifo->event_frame_number() == fem::kINVALID_WORD)

      out_event_tpcfifo->set_event_frame_number ( in_event_tpcfifo->event_frame_number() );

    if(out_event_tpcfifo->fem_trig_frame_number() == fem::kINVALID_WORD)

      out_event_tpcfifo->set_fem_trig_frame_number ( in_event_tpcfifo->fem_trig_frame_number() );

    if(out_event_tpcfifo->fem_trig_sample_number_RAW() == fem::kINVALID_WORD)
